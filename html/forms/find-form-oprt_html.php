...
<div class="row justify-content-center">
    <div class="find-form-wrapper bg-light mt-2">
        <h3 class="text-center mt-2 mb-3">Определить талон(ы)</h3>
        <form method="POST" onsubmit="return false" name="bc" role="form">
            <div class="form-group  pl-2 pr-2">
                <input type="text" name="js-barcode" id="js-barcode" pattern="^[ 0-9]+$" class="form-control" minlength="13" maxlength="13" autocomplete="off" placeholder="Введите(отсканируйте) штрих-код" autofocus>
                <button hidden id="js-btn"></button>
            </div>
        </form>
        <form method="POST">
            <div class="form-group pl-2 pr-2 mb-5 mt-5">
                <input type="hidden" name="barcode" id="js-post"></input>
                <div class="row">
                    <div class="col">
                        <input class="btn btn-success text-custom btn-block btn-sm" type="submit" name="find_talon_oprt" value="Найти талон(ы)" id="btn">
                    </div>
...
<?php
if ($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['find_talon_oprt'])) {
    unset($_POST["find_talon_oprt"]);
    require_once "../php/find-oprt.php";
}
