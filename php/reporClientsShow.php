...
$clnt_name = $_POST['clnt_name'];
$startDate = $_POST['startDate'];
$finishDate = $_POST['finishDate'];
$typeReport = $_POST['typeReport'];
$all = $_POST['all'];

unset($_SESSION['signInvoice']);
...
if ($_POST['invoice'] != "") {
    ...
    $_SESSION['signInvoice'] = true;

    $invoice = mysqli_real_escape_string($link, $_POST['invoice']);

    $query = "SELECT * FROM  `talons` WHERE (`invoice` = '$invoice') 
                                        AND (`clients_name`='$clnt_name')
    ...
    $showResult = "<script type=\"text/javascript\">
                                window.location = '../html/reportClients.php'
                            </script>";
...
if ($all == "allTime") {
    ...
    $queryAll = "SELECT * FROM `talons` WHERE `clients_name`='$clnt_name' ORDER BY `date_give`";
    ...
    if ($typeReport == 'brief') {
        ...
        if (isset($_POST['exportToExcel'])) {
            unset($_post['exportToExcel']);
            clientsAlltimeBriefExcel();
            goto exitScript;
        }

        echo "<fieldset id=\"light\">
        <table class=\"total-table\">
                <caption id=\"total\">За все время клиенту (" . $clnt_name . ")
                    было ВЫДАНО талонов - <u>$numberTotal шт</u> , из них:</caption>
                    <tbody>
                        <tr id=\"tota\">
                            <td></td>
                            <td>ДТ-10л</td>
        ...
        echo "<fieldset id=\"light\"><table class=\"total-table\">
                <caption id=\"return\">ПОГАШЕНО талонов - <u>$number_0 шт</u> , из них:</caption>
                <tbody>
                    <tr id=\"return\">
                        <td></td>
                        <td>ДТ-10л</td>
        ...
        echo "<fieldset id=\"light\"><table class=\"total-table\">
                <caption id=\"give\">Осталось \"НА РУКАХ\" у клиента (" . $clnt_name . "): <u>$number_1 шт</u> талонов, из них:</caption>
                <tbody>
                    <tr id=\"give\">
                        <td></td>
                        <td>ДТ-10л</td>
        ...
        goto exitScript;
    } elseif ($typeReport == 'full') {

        if (isset($_POST['exportToExcel'])) {
            unset($_post['exportToExcel']);
            clientsAlltimeFullExcel();
            goto exitScript;
        }
...
if (!empty($clnt_name) && !empty($startDate) && !empty($finishDate) && !empty($typeReport)) {
    ...
    $queryAll = "SELECT *
                    FROM `talons` WHERE `clients_name`='$clnt_name'
    ...
    if ($sql != false) {
        if (mysqli_affected_rows($link) > 0) {
            while ($row = mysqli_fetch_assoc($sql)) {
                array_push($resultArrAll, $row); //заполняем массив всеми найденными значениями
            }
        } else {
            echo ("<span class=\"alert\">Искомых значений не найдено...попробуйте изменить фильтр выборки отчета.</span>");
            goto exitScript;
    ...
    for ($i = 0, $count = count($resultArrAll); $i < $count; $i++) {

        ///////////////////////////находим общее кол-во выданных за период талонов///////////////////////////////////////
        if ($resultArrAll["$i"]['date_give'] >= $startDate && $resultArrAll["$i"]['date_give'] <= $finishDate) {
            switch (true) {
                case preg_match('~2092010[0-9]{6}~', $resultArrAll["$i"]['barcode']):
                    $a9210 = $a9210 + 1;
                    $numberTotal = $numberTotal + 1;
                    break;
                case preg_match('~2092020[0-9]{6}~', $resultArrAll["$i"]['barcode']):
                    $a9220 = $a9220 + 1;
                    $numberTotal = $numberTotal + 1;
                    break;
                case preg_match('~2095010[0-9]{6}~', $resultArrAll["$i"]['barcode']):
                    $a9510 = $a9510 + 1;
                    $numberTotal = $numberTotal + 1;
                    break;
                case preg_match('~2095020[0-9]{6}~', $resultArrAll["$i"]['barcode']):
                    $a9520 = $a9520 + 1;
                    $numberTotal = $numberTotal + 1;
                    break;
                case preg_match('~2055010[0-9]{6}~', $resultArrAll["$i"]['barcode']):
                    $dt10 = $dt10 + 1;
                    $numberTotal = $numberTotal + 1;
                    break;
                case preg_match('~2055020[0-9]{6}~', $resultArrAll["$i"]['barcode']):
                    $dt20 = $dt20 + 1;
                    $numberTotal = $numberTotal + 1;
                    break;
                case preg_match('~2078010[0-9]{6}~', $resultArrAll["$i"]['barcode']):
                    $gaz10 = $gaz10 + 1;
                    $numberTotal = $numberTotal + 1;
                    break;
                case preg_match('~2078020[0-9]{6}~', $resultArrAll["$i"]['barcode']):
                    $gaz20 = $gaz20 + 1;
                    $numberTotal = $numberTotal + 1;
                    break;
            }
    ...
    ///////////////////////////находим общее кол-во "на руках" на конец периода талонов///////////////////////////////////////
    for ($i = 0, $count = count($resultArrAll); $i < $count; $i++) {
        if ($resultArr["$i"]['status'] == 1) {
            switch (true) {
                case preg_match('~2092010[0-9]{6}~', $resultArr["$i"]['barcode']):
        ...
        ///////////////////////////находим общее кол-во "погашенных" на периода талонов///////////////////////////////////////
        if ($resultArr["$i"]['status'] == 0) {
            switch (true) {
                case preg_match('~2092010[0-9]{6}~', $resultArr["$i"]['barcode']):
                    $a9210_0 = $a9210_0 + 1;
    ...
    if ($typeReport == 'brief') {
        ...
        if (isset($_POST['exportToExcel'])) {
            unset($_post['exportToExcel']);
            clientsPeriodBriefExcel();
            goto exitScript;
        }

        echo "<fieldset id=\"light\"><table class=\"total-table\">
                <caption id=\"total\">Всего за период (" . $_POST['startDate'] . " : " . $_POST['finishDate'] . ")
                    клиенту (" . $clnt_name . ") ВЫДАНО талонов - <u>$numberTotal шт</u> , из них:</caption>
                    <tbody>
                        <tr id=\"tota\">
                            <td></td>
                            <td>ДТ-10л</td>
        ...
        echo "<fieldset id=\"light\"><table class=\"total-table\">
                <caption id=\"return\">Всего за период (" . $_POST['startDate'] . " : " . $_POST['finishDate'] . ")
                    клиентом (" . $clnt_name . ") ПОГАШЕНО талонов - <u>$number_0 шт</u> , из них:</caption>
                <tbody>
                    <tr id=\"return\">
                        <td></td>
                        <td>ДТ-10л</td>
        ...
        echo "<fieldset id=\"light\"><table class=\"total-table\">
                <caption id=\"give\">Осталось \"НА РУКАХ\" у клиента (" . $clnt_name . "): <u>$number_1 шт</u> талонов, 
                    на конец периода (" . $_POST['startDate'] . " : " . $_POST['finishDate'] . "), из них:</caption>
                <tbody>
                    <tr id=\"give\">
                        <td></td>
                        <td>ДТ-10л</td>
...
$tmp .= '</div>
        ...
        $_SESSION['typeReport'] = $typeReport;
        $_SESSION['thead'] = $thead;
        $_SESSION['resultArrTbl'] = $resultArrTbl;
        $_SESSION['endTable'] = $endTable;
        $_SESSION['clnt_name'] = $clnt_name;
